...
!(function ($) {
  $.fn.least = function (options) {
    var settings = $.extend(
      { random: !0, scrollToGallery: !0, HiDPI: !0 },
      options
    );
      ...
      function intipreview(object, path, caption) {
        ...
        img.load(function () {
          caption.length
            ...
            : object.html(""),
            object.prepend(img).append(close).slideDown("slow"),
      ...
      if (
        ($(this)
          ...
          .click(function () {
            ...
            return previewImg.length && path === previewImg.attr("src")
              ? (preview.slideToggle("slow"), $$.toggleClass("active"), void 0)
              : (previewImg.length
                  ? ($$.addClass("active"),
                    $(".active").removeClass("active"),
                    preview.slideUp("slow", function () {
                      intipreview(preview, path, caption);
                    }))
                  : intipreview(preview, path, caption),
        ...
        settings.random &&
          $(".least-gallery").each(function () {
            var ul = $(this),
              li = ul.children("li");
            li.sort(function () {
              var temp = parseInt(8 * Math.random(), null),
                OddEven = temp % 4,
                PosNeg = temp > 2 ? 1 : -1;
              return OddEven * PosNeg;
            }).appendTo(ul);
          }),
        settings.scrollToGallery &&
          $(this)
            .find("li a")
            .click(function (e) {
              e.preventDefault(),
        ...
        settings.HiDPI && window.devicePixelRatio > 1)
      )
        for (
          var image_thumb = $("#least img"), image_big = $("#least a"), i = 0;
          i < image_thumb.length && image_big.length;
          i++
        ) {
